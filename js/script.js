...
(function() {
    ...
    function updateCountryNote() {
        ...
        else if (country === 'sd') noteKey = 'sdNote';
        else noteKey = 'maNote';
    ...
    async function updateNisab() {
        ...
        const preferredNisab = selected.dataset.preferredNisab;

        // الأسعار الافتراضية (الاحتياطية)
        let goldPrice = parseFloat(selected.dataset.goldPrice) || 500;
        let silverPrice = parseFloat(selected.dataset.silverPrice) || 26;
        let sourceText = 'افتراضي';

        // إذا لم يكن المستخدم في الوضع اليدوي، حاول جلب الأسعار الحية
        if (!useManualPrices) {
            ...
            if (metalPrices && fixerRates) {
                ...
                // حساب سعر الصرف من الدولار إلى العملة المحلية
                const eurToUsd = fixerRates['USD'] ? 1 / fixerRates['USD'] : 1;
                const localPerEur = fixerRates[currency];
                if (localPerEur && eurToUsd) {
                    const usdToLocal = localPerEur * eurToUsd;
                    goldPrice = metalPrices.gold * usdToLocal;
            ...
            // استخدام الأسعار اليدوية
            if (manualGoldPrice.value) goldPrice = parseFloat(manualGoldPrice.value);
            if (manualSilverPrice.value) silverPrice = parseFloat(manualSilverPrice.value);
            sourceText = currentLang === 'ar' ? 'يدوي' : 'Manual';
        }

        // حفظ الأسعار الحالية
        currentPrices.gold = goldPrice;
        currentPrices.silver = silverPrice;
        ...
        priceSource.innerText = sourceText;
        updateStatus.innerText = currentLang === 'ar' ? '✓ محدث' : '✓ Updated';
        updateStatus.style.background = 'var(--md-primary)';
